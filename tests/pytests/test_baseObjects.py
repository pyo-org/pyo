...
class TestPyoObject:
    ...
    def test_out(self, audio_server):
        ...
        audio_server.shutdown()
        audio_server.reinit(sr=48000, buffersize=512, nchnls=8, audio="offline")
        audio_server.recordOptions(dur=2)
        audio_server.boot()
...
class TestPyoTableObject:
    ...
    def test_save(self, audio_server):
        path = os.path.join(os.path.expanduser("~"), "temporary_soundfile.wav")

        t = HarmTable()
        t.save(path, format=0, sampletype=3)
        info = sndinfo(path)
        assert info[0] == 8192
        assert info[2] == audio_server.getSamplingRate()
        assert info[3] == 1
        assert info[4] == "WAVE"
        assert info[5] == "32 bit float"
        t2 = SndTable(path)
        assert t.getSize() == t2.getSize()
        for i in range(t.getSize()):
            assert t.get(i) == t2.get(i)
    ...
    def test_getSize(self):
        ...
        assert t.getSize(all=True) == [66213, 66213]
